...
3. the $\omega$ is convex. From this we  get that for any $\boldsymbol{\tau}\in \mathcal S^{\infty}_n$ its segmentation cost is the sum of segment cost functions:
...
\Procedure{GeomFPOP}{$y, \Omega(\cdot, \cdot),\beta$}
  ...
  \For{$t = 1, \dots, n$}
    ...
    \For{$i-1 \in \tau_{t-1}$}
      ...
      \EndIf
    \EndFor
    \State $\tau_{t-1} \gets (\tau_{t-1}, t-1)$
  \EndFor
...
We define a cost function for segmentation as in @eq-Cy_it by the function $\Omega(\cdot,\cdot)$ (the negative log-likelihood (times two)). Below is the expression of this function  linked to data point $y_i = (y_i^1,\dots, y_i^p)$ in $\mathbb R^p$ for three examples of parametric multi-variate models:
...
![](images/Figure12_new_appendix_robustness.png){width=80%}
